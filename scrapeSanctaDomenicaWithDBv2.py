...
def create_table():
    conn = sqlite3.connect("productsV2.db")
    c = conn.cursor()
...
def insert_data(name, price, screen_size, manufacturer, screen_type, tv_code):
    conn = sqlite3.connect("productsV2.db")
    c = conn.cursor()
    try:
        c.execute(
            "INSERT INTO productsSanctaDomenica (name, price, screen_size, manufacturer, screen_type, tv_code) VALUES (?, ?, ?, ?, ?, ?)",
            (name, price, screen_size, manufacturer, screen_type, tv_code))
        conn.commit()
...
def extract_products(soup):
    ...
    for item in product_items:
        try:
            ...
            products.append((name, price_float, screen_size, manufacturer, screen_type, tv_code))
...
while current_url:
    ...
    for name, price, screen_size, manufacturer, screen_type, tv_code in products:
        if not product_exists(name):
            insert_data(name, price, screen_size, manufacturer, screen_type, tv_code)
            print("Name:", name)
